describe('The dp-menu-dropdown directive', function () {
    ...
    function getDirective (options, title = 'Menu', type = 'main') {
        var directive,
            element,
            scope;
    ...
    describe('the main menu', function () {
        ...
        it('should toggle the visibility of the menu items when you click menu button', function () {
            ...
            expect(directive.find('.menu-dropdown__contents').length).toBe(1);
        ...
        it('supports multiple, standalone, dropdown menu\'s on one page', function () {
            ...
            expect(directive1.find('.menu-dropdown__contents').length).toBe(0);
            expect(directive2.find('.menu-dropdown__contents').length).toBe(0);

            directive1.find('.site-header__menu__item--toggle').eq(0).click();
            expect(directive1.find('.menu-dropdown__contents').length).toBe(1);
            expect(directive2.find('.menu-dropdown__contents').length).toBe(0);

            directive2.find('.site-header__menu__item--toggle').eq(0).click();
            expect(directive1.find('.menu-dropdown__contents').length).toBe(0);
            expect(directive2.find('.menu-dropdown__contents').length).toBe(1);
        });
        ...
        it('has an option to show/hide the print button', function () {
            ...
            directive.find('.site-header__menu__item--toggle').eq(0).click();
            expect(directive.find('.menu-dropdown__contents').text()).toContain('Printen');

            // Without a print button
            directive = getDirective({hasPrintButton: false});
            directive.find('.site-header__menu__item--toggle').eq(0).click();
            expect(directive.find('.menu-dropdown__contents').text()).not.toContain('Printen');
        });
        ...
        it('has an option to download adressen', function () {
            ...
            directive = getDirective({hasPrintButton: false});
            directive.find('.site-header__menu__item--toggle').eq(0).click();
            expect(directive.find('.menu-dropdown__contents').text()).toContain('Downloaden adressen');
        });
    });
    ...
    describe('the user menu', function () {
        ...
        it('provides for a logout menu item', function () {
            ...
            // Open the menu
            directive.find('.qa-menu-title').eq(0).click();
