...
describe('Filters', () => {
  ...
  describe('modification date filter', () => {
    ...
    it('shows the time as since created: today / 0 days', () => {
      expect(modificationDateFilter('2018-10-05')).toBe('vandaag gewijzigd');
    });

    it('shows the time as since created: yesterday / 1 days', () => {
      expect(modificationDateFilter('2018-10-04')).toBe('gisteren gewijzigd');
    });

    it('shows the time as since created: 4 days', () => {
      expect(modificationDateFilter('2018-10-01')).toBe('4 dagen geleden gewijzigd');
    });

    it('shows the time as since created: future time', () => {
      expect(modificationDateFilter('2018-11-04')).toBe('in de toekomst gewijzigd');
    });
  });
  ...
  describe('truncate filter', () => {
    ...
    it('caps the resulting text to a maximum of 250 characters', () => {
      for (let i = MAX_LENGTH - 25; i < MAX_LENGTH + 25; i += 1) {
        expect(truncateHtmlAsTextFilter('a'.repeat(i)).length).toBe(i <= MAX_LENGTH ? i : MAX_LENGTH);
      }
    });

    it('adds ... to a text if it is capped', () => {
      for (let i = MAX_LENGTH + 1; i < MAX_LENGTH + 25; i += 1) {
        expect(truncateHtmlAsTextFilter('a'.repeat(i))).toBe('a'.repeat(MAX_LENGTH - ELLIPSES.length) + ELLIPSES);
      }
    });
    ...
    it('caps the text on the last space, if a space is available', () => {
      expect(truncateHtmlAsTextFilter('a'.repeat(MAX_LENGTH - 50) + ' '.repeat(75)).length).toEqual(MAX_LENGTH - 50);
      expect(truncateHtmlAsTextFilter('a'.repeat(MAX_LENGTH - 50) + ' '.repeat(25) + 'a'.repeat(50)).length)
