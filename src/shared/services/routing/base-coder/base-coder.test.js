import BaseCoder, { getCoderForBase } from './base-coder';
...
describe('The BaseCoder', () => {
  ...
  it('encodes and decodes a number in any base between 2 and 62', () => {
    for (let base = 2; base <= 62; base++) { // eslint-disable-line no-plusplus
  ...
  it('can encode an array of an array of numbers', () => {
    expect(encode([[61, -61], [62, -62], [63, -63]], 62)).toEqual([['z', '-z'],
    ['10', '-10'], ['11', '-11']]);
    expect(decode([['z', '-z'], ['10', '-10'], ['11', '-11']], 62)).toEqual([[61, -61],
    [62, -62], [63, -63]]);
  });
