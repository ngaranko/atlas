...
export default function fetchByUri(uri) {
  return getByUrl(uri).then(result => {
    ...
    return result.maatschappelijke_activiteit
      ? maatschappelijkeActiviteit(result.maatschappelijke_activiteit).then(
          mac => {
            const special = vestigingResult._bijzondere_rechts_toestand || {}
            return {
              ...vestigingResult,
              activities: (vestigingResult.activiteiten || []).map(
                activity => ({
                  ...activity,
              ...
              bijzondereRechtstoestand: {
                ...
                surseanceVanBetaling:
                  special.status === 'Voorlopig' ||
                  special.status === 'Definitief',
              },
...
export function fetchByPandId(pandId) {
  ...
  return getByUrl(
    `${SHARED_CONFIG.API_ROOT}handelsregister/vestiging/?${queryString}`,
