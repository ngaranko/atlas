...
import { MAP_BOUNDING_BOX } from '../../../map/ducks/map/constants'
import {
  closeMapPanel,
  mapEmptyGeometry,
  mapEndDrawing,
...
function* retrieveDataSelection(action) {
  const { searchText, catalogFilters } = action.payload
  ...
  try {
    ...
    const activeAttributeFilters = Object.keys(activeFilters).reduce(
      (result, key) => ({
        ...result,
