...
describe('watchErrors', () => {
  it('should watch the error actions and call set errors', () => {
    ...
    testSaga(watchErrors)
      ...
      .next(action)
      .takeLatestEffect([FETCH_DATA_SELECTION_FAILURE],
        excludeUnauthorizedErrorEffect)
      .next(action)
      .isDone();
  });
...
describe('excludeUnauthorizedErrorEffect', () => {
  it('should call the set errors effect when authorized', () => {
    ...
    testSaga(excludeUnauthorizedErrorEffect, mockAction).next()
      .call(setErrorsEffect).next()
      .isDone();
  });
...
describe('setErrorsEffect', () => {
  it('should dispatch the global error', () => {
    testSaga(setErrorsEffect).next()
      .put(setGlobalError(ERROR_TYPES.GENERAL_ERROR)).next()
      .isDone();
  });
});
