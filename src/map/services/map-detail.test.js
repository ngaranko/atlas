...
describe('The map detail service', () => {
  ...
  it('calls the service fot the endpoint type specified', async () => {
    ...
    expect(await detail('https://acc.api.data.amsterdam.nl/bag/ligplaats/123'))
      .toEqual({ endpointType: 'bag/ligplaats/', type: 'ligplaats' });
    expect(adressenLigplaats)
      .toHaveBeenCalledWith('https://acc.api.data.amsterdam.nl/bag/ligplaats/123', undefined);

    napPeilmerk.mockImplementation(() => ({ type: 'peilmerk' }));
    expect(await detail('https://acc.api.data.amsterdam.nl/nap/peilmerk/123'))
      .toEqual({ endpointType: 'nap/peilmerk/', type: 'peilmerk' });
    expect(napPeilmerk)
      .toHaveBeenCalledWith('https://acc.api.data.amsterdam.nl/nap/peilmerk/123', undefined);
  });

  describe('endpoint type nummeraanduiding', () => {
    it('returns endpoint type verblijfsobject by default', async () => {
      adressenNummeraanduiding.mockImplementation(() => ({ type: 'nummeraanduiding' }));
      expect(await detail('https://acc.api.data.amsterdam.nl/bag/nummeraanduiding/123'))
        .toEqual({ endpointType: 'bag/verblijfsobject/', type: 'nummeraanduiding' });
      expect(adressenNummeraanduiding)
        .toHaveBeenCalledWith('https://acc.api.data.amsterdam.nl/bag/nummeraanduiding/123', undefined);
    });

    it('returns endpoint type ligplaats when it is a ligplaats', async () => {
      adressenNummeraanduiding.mockImplementation(() => ({ type: 'nummeraanduiding', ligplaats: true }));
      expect(await detail('https://acc.api.data.amsterdam.nl/bag/nummeraanduiding/123'))
        .toEqual({ endpointType: 'bag/ligplaats/', type: 'nummeraanduiding', ligplaats: true });
      expect(adressenNummeraanduiding)
        .toHaveBeenCalledWith('https://acc.api.data.amsterdam.nl/bag/nummeraanduiding/123', undefined);
    });

    it('returns endpoint type standplaats when it is a standplaats', async () => {
      adressenNummeraanduiding.mockImplementation(() => ({ type: 'nummeraanduiding', standplaats: true }));
      expect(await detail('https://acc.api.data.amsterdam.nl/bag/nummeraanduiding/123'))
        .toEqual({ endpointType: 'bag/standplaats/', type: 'nummeraanduiding', standplaats: true });
      expect(adressenNummeraanduiding)
        .toHaveBeenCalledWith('https://acc.api.data.amsterdam.nl/bag/nummeraanduiding/123', undefined);
    });
  });
  ...
  it('calls the service when the user has the required auth scope', async () => {
    ...
    expect(await detail('https://acc.api.data.amsterdam.nl/handelsregister/vestiging/123', user))
      .toEqual({ endpointType: 'handelsregister/vestiging/', type: 'vestiging' });
    expect(vestiging)
      .toHaveBeenCalledWith('https://acc.api.data.amsterdam.nl/handelsregister/vestiging/123', user);
  });
});
