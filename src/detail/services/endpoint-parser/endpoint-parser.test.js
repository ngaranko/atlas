...
describe('The endpointParser', () => {
  describe('getParts', () => {
    it('returns the category and subject based on an endpoint', () => {
      expect(getParts('https://api.data.amsterdam.nl/bag/nummeraanduiding/123456/')).toEqual([
        'bag',
        'nummeraanduiding',
      ])

      expect(getParts('https://api.data.amsterdam.nl/brk/object/789/')).toEqual(['brk', 'object'])

      expect(getParts('https://api.data.amsterdam.nl/meetbouten/meetbout/654/')).toEqual([
        'meetbouten',
        'meetbout',
      ])

      expect(
        getParts(`https://api.data.amsterdam.nl/brk/object-wkpb/NL.KAD.OnroerendeZaak.123456/`),
      ).toEqual(['brk', 'object-wkpb'])

      expect(getParts('https://api.data.amsterdam.nl/folder-1/folder-2/folder-3/123/')).toEqual([
        'folder-2',
        'folder-3',
      ])
  ...
  describe('getTemplateUrl', () => {
    it('returns a template URL based on an endpoint', () => {
      expect(getTemplateUrl('https://api.data.amsterdam.nl/bag/nummeraanduiding/123456/')).toBe(
        'modules/detail/components/detail/templates/bag/nummeraanduiding.html',
      )

      expect(getTemplateUrl('https://api.data.amsterdam.nl/brk/object/789/')).toBe(
        'modules/detail/components/detail/templates/brk/object.html',
      )

      expect(getTemplateUrl('https://api.data.amsterdam.nl/meetbouten/meetbout/654/')).toBe(
        'modules/detail/components/detail/templates/meetbouten/meetbout.html',
      )

      expect(getTemplateUrl('https://api.data.amsterdam.nl/folder-1/folder-2/folder-3/654/')).toBe(
    ...
    it('has special exception for catalogus detail pageuss', () => {
      expect(getTemplateUrl('https://data.amsterdam.nl/dcatd/datasets/id')).toBe(
        'modules/detail/components/detail/templates/dcatd/datasets.html',
      )
    })
