    ...
    stage("Build image") {
        tryStep "build", {
            withCredentials([[$class: 'StringBinding', credentialsId: 'PASSWORD_EMPLOYEE', variable: 'PASSWORD_EMPLOYEE'],
                ...
                sh "echo PIPOMARKER -e PASSWORD_EMPLOYEE=$PASSWORD_EMPLOYEE -e PASSWORD_EMPLOYEE_PLUS=$PASSWORD_EMPLOYEE_PLUS --entrypoint 'grunt test-e2e'"
                image.inside('grunt test-e2e')
            }
            image.push()
        }
    }
